...
class bbpress_like {
	...
	function __construct(){
		...
		global $wpdb;
		$this->plugin_name = 'bbpress-like-button-edited';
		...
		add_action('wp_ajax_unlike_this', array($this, 'unlike_this'));
		add_action('wp_ajax_nopriv_unlike_this', array($this, 'unlike_this'));
	...
	function admin_menu(){
		...
		$pages = array();
		$pages[] = add_menu_page(__('Likes', $this->l10n), __('Likes', $this->l10n), 'add_users', 'bbpress-likes', array(
			$this,
			'admin_screen'
		), plugins_url('bbpress-like-button/img/thumbs_up_15_15.png'), 100);
		$pages[] = add_submenu_page('bbpress-likes', __('Likes logs', $this->l10n), __('Likes logs', $this->l10n), 'add_users', 'bbpress-likes-logs', array(
			$this,
			'logs_screen'
		));
		$pages[] = add_submenu_page('bbpress-likes', __('Likes statistics', $this->l10n), __('Likes statistics', $this->l10n), 'add_users', 'bbpress-likes-statistics', array(
			$this,
			'statistics_screen'
		));
	...
	function load_labels(){
		...
		$this->labels = array(
			"action_l" => "Like this",
			"action_done_l" => "You liked this",
			"action_ul" => "Not like this",
			"action_done_ul" => "You not liked this"
		);
		//User defined
		...
		if($this->settings['settingsbbpl_bbpl_labels_label_unlike_button_action'] != "") {
			$this->labels['action_ul'] = $this->settings['settingsbbpl_bbpl_labels_label_like_button_action'];
		}

		if($this->settings['settingsbbpl_bbpl_labels_label_unlike_button_action_done'] != "") {
			$this->labels['action_done_ul'] = $this->settings['settingsbbpl_bbpl_labels_label_like_button_action_done'];
		}
	}
	...
	function admin_enqueue_scripts(){
		...
		wp_enqueue_script('jquery.cookie', plugins_url('bbpress-like-button/js/jquery.cookie.js'), 'jquery', '1.2', true);
		wp_enqueue_script('flexigrid', plugins_url('bbpress-like-button/js/flexigrid.js'), 'jquery', '1.1', true);
		wp_enqueue_script('bbpl-functions', plugins_url('bbpress-like-button/js/admin_functions.js'), '', '1.1', true);
		wp_enqueue_script('jquery.tools.tooltip', plugins_url('bbpress-like-button/js/jquery.tools.min.tooltip.js'), 'jquery', '1.2.7', true);
	}

	function public_enqueue_scripts(){

		wp_enqueue_script('bbpl-functions', plugins_url('bbpress-like-button/js/public_functions.js'), '', '1.1', true);
		wp_enqueue_script('jquery.tools.tooltip', plugins_url('bbpress-like-button/js/jquery.tools.min.tooltip.js'), 'jquery', '1.2.7', true);
	}
	...
	function get_like($post_id, $user_id){
		...
		$result = $wpdb->get_row($wpdb->prepare("SELECT
            ...
            WHERE
                meta_key = 'bbpl_like'
                AND post_id = %d
                 AND meta_value = %d", $post_id, $user_id));

		return $result;
	}

	function get_unlike($post_id, $user_id){
	...
	function get_unlikes_number($post_id){
	...
	function  get_sum_likes($post_id){

		return $this->get_likes_number($post_id) - $this->get_unlikes_number($post_id);
	}
	...
	function like_this(){
		...
		if($liked) {
			...
			//Insert like
			if(self::get_unlike($post_id, $user_id)) {
				$result = delete_post_meta($post_id, 'bbpul_like', $user_id);
			}
			else {
				$result = add_post_meta($post_id, 'bbpl_like', $user_id);
			}
			if(!$result) {
				'!' . _e('An error ocurred: ', $this->l10n) . $wpdb->print_error();
			}
			else {
				echo $this->get_sum_likes($post_id);
			}
	...
	function unlike_this(){
		...
		//Check if previously liked
		$liked = self::get_unlike($post_id, $user_id);
		if($liked) {
			_e('This post was already liked by this user', $this->l10n);
		}
		else {
			//Insert like
			if(self::get_like($post_id, $user_id)) {
				$result = delete_post_meta($post_id, 'bbpl_like', $user_id);
			}
			else {
				$result = add_post_meta($post_id, 'bbpul_like', $user_id);
			}
			if(!$result) {
	...
	function get_most_liked_users($echo = true, $exclude_admins = false, $get_only_data = false){
		...
		if($result) {
			...
			foreach($result as $liked_users) {
				...
				if($exclude_admins && $user->caps['administrator'] == 1)
					continue;
	...
	public function bbpl_show_button($echo = true){
		...
		$unliked = ($this->get_unlike($post_id, $user_id)) == null ? false : true;

		$link_caption = ($liked == true ? __($this->labels["action_done"], $this->l10n) : __($this->labels["action"], $this->l10n));

		$unlink_caption = ($unliked == true ? __($this->labels["action_done_ul"], $this->l10n) : __($this->labels["action_ul"], $this->l10n));

		$label_link_caption = $link_caption;
		$label_unlink_caption = $unlink_caption;
		if(!(Bool) $this->settings['settingsbbpl_bbpl_general_show_like_button_label']) {
			$label_link_caption = '&nbsp;';
			$label_unlink_caption = '&nbsp;';
		}

		ob_start();
		?>
		<div class="bbpl_wrapper">
			<?php
			$like_number = $this->get_sum_likes($post_id);
			if((Bool) $this->settings['settingsbbpl_bbpl_general_show_number'] ) {
				?>
			...
			<div class="bbpl_button_wrapper">
				<a href="#" data-user="<?php echo $user_id; ?>" data-post="<?php echo $post_id; ?>"
				   title="<?php echo $link_caption; ?>"
				   class="bbpl_button <?php echo $liked == true ? 'liked' : '' ?>"><span><?php echo $label_link_caption; ?></span></a>
			</div>
			<?php
			...
			?>
			<div class="bbpul_button_wrapper">
				<a href="#" data-user="<?php echo $user_id; ?>" data-post="<?php echo $post_id; ?>"
				   title="<?php echo $unlink_caption; ?>"
				   class="bbpul_button <?php echo $unliked == true ? 'liked' : '' ?>"><span><?php echo $label_unlink_caption; ?></span></a>
			</div>
		</div>
	<?php
	}
